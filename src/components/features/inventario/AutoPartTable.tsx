...
// anidado en la relación, así que la columna se arma contra el catálogo de
// categorías — mismo patrón que /inventario/[id] y /inventario/importar.
function buildColumns(categorias: Categoria[]) {
  return [
    ...
    columnHelper.accessor('subcategoria', {
      id: 'categoria',
      header: 'Categoría',
      cell: (info) => {
        const subcategoria = info.getValue();
        const nombre = categorias.find((c) => c.id === subcategoria?.categoriaId)?.nombre;
        return nombre ? (
...
export function AutoPartTable({ data, categorias }: AutoPartTableProps) {
  const columns = useMemo(() => buildColumns(categorias), [categorias]);
